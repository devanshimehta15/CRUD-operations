...
app.post('/update',(req,res) => {
	...
	db.collection('bata').find().toArray((err,result) => {
		...
		db.collection('bata').findOneAndUpdate({pid:req.body.pid},{
		    $set: {pstock: parseInt(s) + parseInt(req.body.pstock)}}, {sort: {_pid:-1}},
			(err,result) => {
			if(err)
				return res.send(err)
...
app.post('/delete',(req,res) => {
	db.collection('bata').findOneAndDelete({pid:req.body.pid}, (err,result)=>{
		if(err)
			return console.log(err)
		res.redirect('/')
	})
